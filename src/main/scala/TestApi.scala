...
//    connection.setRequestProperty("Authorization","Basic " +decodedAuth)
    ...
    log.debug("Starting the input readstream")
    var b = inputdata.read()
    while(b != -1) {
      outputdata.write(b)
      b = inputdata.read()
    }
    log.debug("Input stream written into the application.")
    outputdata.close()
    inputdata.close()
  }
